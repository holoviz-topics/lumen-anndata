...
from lumen.ai.llm import LlamaCpp
...
async def start():
    ...
    vector_store = DuckDBVectorStore(uri=db_path, llm=LlamaCpp(), embeddings=HuggingFaceEmbeddings(), chunk_size=256)
    print(db_path)
    await vector_store.add_directory(f"scanpy_{VERSION}/html", pattern="*.html", metadata={"version": VERSION}, situate=False)
